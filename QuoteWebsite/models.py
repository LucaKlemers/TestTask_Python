...
class QuoteSource(models.Model):
    ...
    def clean(self):  # Check if valid
        if self.publishment_date > timezone.now().year and bool(
            self.published_AC
        ):  # Check if publishment year is real
            raise ValidationError(
                {"publishment_date": "Нельзя процитировать источник из будущего"}
            )

        def text_clean(string):
            return "".join(
                e for e in string if e.isalnum()
            ).lower()  # Clean the quote from special characters, whitespaces. Make lowercase

        cleaned_sources = []
        for string in QuoteSource.objects.exclude(
            pk=self.pk
        ):  # Creating a list of clean sources
...
class Quote(models.Model):
    ...
    def save(self, *args, **kwargs):
        other_quotes = (
            ...
            .exclude(pk=self.pk)
            .order_by("total_rating", "quote_weight")
        )
        if len(other_quotes) >= 3:
            other_quotes.first().delete()
        self.total_rating = (
            self.positive_rating - self.negative_rating
        )  # Update the total rating after rating changes
    ...
    def clean(self):  # Check if valid
        ...
        cleaned_quotes = []
        for string in Quote.objects.exclude(
            pk=self.pk
        ):  # Creating a list of clean quotes
        ...
        if self.quote_weight > 100 or self.quote_weight < 1:
            ...
            raise ValidationError({"quote_text": "Эта цитата уже добавлена"})
    ...
    def check_vote(self, request):
        cookie = f'{self.pk}_vote'
